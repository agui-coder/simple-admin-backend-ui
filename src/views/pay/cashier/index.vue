<script lang="ts">
  ...
  import { OrderSubmitReq }from '@/api/pay/model/orderModel';
  import { PayChannelEnum, PayDisplayModeEnum, PayOrderStatusEnum } from '@/enums/payEnum'
  ...
  export default defineComponent({
    name: 'Cashier',
    components:{Card,CashierModal,Description,Meta},
    setup() {
      const go = useGo();
      ...
      async function submit0(channelCode) {
        ...
        try {
          const orderSubmitRequest: OrderSubmitReq = {
            id: parseInt(id.value, 10),
            channelCode,
            returnUrl: location.href, // 支付成功后，支付渠道跳转回当前页；再由当前页，跳转回 {@link returnUrl} 对应的地址
            ...buildSubmitParam(channelCode),
          };
          const resp = await submitOrder(orderSubmitRequest);
          // 直接返回已支付的情况，例如说扫码支付
          if (resp.status === PayOrderStatusEnum.SUCCESS.status) {
            clearQueryInterval();
          ...
          // 展示对应的界面
          if (resp.displayMode === PayDisplayModeEnum.URL.mode) displayUrl(channelCode, resp);
          else if (resp.displayMode === PayDisplayModeEnum.QR_CODE.mode)
            displayQrCode(channelCode, resp);
          else if (resp.displayMode === PayDisplayModeEnum.APP.mode) displayApp(channelCode);
      ...
      function displayUrl(_channelCode, data) {
        console.info(data.displayContent)
      ...
      function displayQrCode(channelCode, data) {
        ...
        } else if (channelCode.indexOf('Alipay') === 0) {
          title = '请使用支付宝“扫一扫”扫码支付';
        } else if (channelCode.indexOf('Wx') === 0) {
      ...
      function createQueryInterval() {
        ...
        interval.value = setInterval(async () => {
          const resp = await getOrder({ id: Number(id.value) });
          // 已支付
          if (resp.data.status === PayOrderStatusEnum.SUCCESS.status) {
            clearQueryInterval();
            createMessage.success('支付成功！');
            goReturnUrl('success');
          }
          // 已取消
          if (resp.data.status === PayOrderStatusEnum.CLOSED.status) {
            clearQueryInterval();
      ...
      async function getDetail() {
        ...
        const res = await getOrder({ id: Number(id.value) });
...
<template>
  <div class="p-4">
    ...
    <Card title="支付" class="mt-4">
      <p class="mb-4 text-lg text-dark-900"> 选择支付宝支付 </p>
      <div class="flex">
        <Card
        ...
        >
          ...
          <Meta :title="alipay.name" class="text-center" />
        </Card>
      </div>
      <p class="mb-4 text-lg text-dark-900"> 选择微信支付 </p>
      <div class="flex">
        <Card
        ...
        >
          ...
          <Meta :title="wechat.name" class="text-center" />
        </Card>
      </div>
      <p class="mb-4 text-lg text-dark-900"> 选择其它支付 </p>
      <div class="flex">
        <Card
        ...
        >
          ...
          <Meta :title="mock.name" class="text-center" />
        </Card>
      </div>
    </Card>
